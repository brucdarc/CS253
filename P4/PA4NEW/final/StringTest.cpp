...
if(isCaps(testChar) || (testChar>96 && testChar<123) || testChar==39) return true;
return false;
}
/*
...
bool StringTest::isValidStart(const string& testStr) 

//checks if the word has one caps at beggining and only one
//and if it is followed by punctuation only

{
if(!isCaps(testStr.at(0))) return false;

for(unsigned int i=1;i<testStr.size();i++)
{
if(!isLetter(testStr.at(i))) return false;
if(isCaps(testStr.at(i))) return false;
}
return true;
}
...
bool StringTest::hasPEQ(const string& testStr)
{
//checks if a string has at least one period, exclamation point and/or questionmark


for(unsigned int i=0;i<testStr.size();i++)
{
char test = testStr.at(i);
if( (test == 46) || (test == 33) || (test == 63))
return true;
}
return false;
}
...
{
//marks valid words that have one caps at start, and follow a punctuation only string
//containing Period, exclamation point, and/or question mark
...
{
if(hasPEQ(words.at(i-1))){

if(isValidStart(words.at(i)))
words[i] = "+" + words[i];
