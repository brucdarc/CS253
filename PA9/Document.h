...
Document (vector<Word> words, double readL, string FN);

		//initialized wds then sorts it to be in lexographical order
...
static int getNumSents(const vector<string> &words);	// returns the number of PEQ punct words that break sentences
static int getNumAlphaWords(const vector<string> &words);	// returns number of alphanumeric words according to definition
static int getNumAlphaWordsChars(const vector<string> &words);	// counts the chars in each alphanumeric word
static double getS(const vector<string> &words);	//returns sentence calculation part of readinglevel calculation
static double getL(const vector<string> &words);	//returns words part of calculation in reading level
static inline bool getFail() {return failed;}	//tells if anything as failed in processes done on the doc
static double getReadingLevel(const vector<string> &words); // returns from equation, .whatever * L - .whatever * S - 15.whatever

inline double RL() {return readingLevel;}
...
double readingLevel;
string fileName;
static bool failed; //keeps tract of if things done on document have failed
